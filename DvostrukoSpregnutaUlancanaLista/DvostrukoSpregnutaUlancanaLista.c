...
void dodaj_na_kraj(PCVOR* glava, int broj)
{
	...
	{
		PCVOR poslednji = (*glava)->prethodni;
...
int izbaci_sa_kraja(PCVOR* glava)
{
	...
	{
		PCVOR poslednji = cvor->prethodni;
		PCVOR pretposlednji = poslednji->prethodni;
		pretposlednji->sledeci = cvor;
		cvor->prethodni = pretposlednji;
		broj = poslednji->info;
		free(poslednji);
	}
	return broj;
...
void izbaci_sa_pozicije(PCVOR* glava, int n)
{
	...
	if (n == 0)
	{
		...
		*glava = (*glava)->sledeci;
		PCVOR prethodni = tekuci->prethodni;
		prethodni->sledeci = *glava;
		(*glava)->prethodni = prethodni;
		free(tekuci);
		return;
	}
	for (int i = 0; i < n; i++)
